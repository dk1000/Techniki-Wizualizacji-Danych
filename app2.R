...
df_avg = read.csv("df_avg.csv", fileEncoding = "cp1250")
srednie = read.csv("srednie.csv", fileEncoding = "cp1250")

SO2 = read.csv("SO2.csv", fileEncoding = "cp1250")
NO2 = read.csv("NO2.csv", fileEncoding = "cp1250")
PM10 = read.csv("PM10.csv", fileEncoding = "cp1250")
PM25 = read.csv("PM25.csv", fileEncoding = "cp1250")
...
server <- function(input, output) {
  ...
  output[["srednie"]] <- renderPlot({
    ...
    df_avg1$gas <- factor(df_avg1$gas, levels = df_avg1$gas[order(df_avg1[,input[["chosen_gas"]]])])
  ...
  output[["wykres_mapa"]] <- renderPlot({
    ...
    names(table) = c("Rok", "Wojewodztwo", "Srednia")
    
    table = select(table, Wojewodztwo, Srednia) %>%
      group_by(Wojewodztwo) %>%
      mutate(mean = mean(Srednia)) %>% select(Wojewodztwo, mean) %>% distinct() %>% arrange(Wojewodztwo)
    
    nazwa_woj = data.frame(woj$jpt_nazwa_)
    names(nazwa_woj) = "Wojewodztwo"
    ...
    klasy = classIntervals(table$mean, 9, style="fixed", 
                           fixedBreaks= round(seq(from=min(as.numeric(table[,2])), to=max(as.numeric(table[,2])), length.out = 9)),2)
